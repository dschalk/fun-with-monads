...
function view(m1, m2, m3, m4, m5, m6, m7, m8, mI1, mI2) { 
  ...
  add = (x,mon,y) => {mon.ret(x + y);
    return mon;
      ...
      h('button', {on: { mouseenter: update7e, mouseleave: update7l, click: update2B }, style: style7},
               `mM1.ret(3).bnd(v => mM2.ret(v).fmap( cu, mM2 ))`  ),  
      h('p', 'Next, we accomplish the same thing without any help from bnd.'  ),
      h('button', {on: { mouseenter: update8e, mouseleave: update8l, click: update2B2 }, style: style8},
      ...
      h('p', 'Here\s one last look at the three monad methods: '  ),                   
      h('p', 'GOAL: give mM8 the value "test" and pass it to other monads using ret, fmap, and bnd. '  ), 
      h('button', {on: { mouseenter: update4e, mouseleave: update4l, click: updateTest }, style: style4},
            [ cow.test ],  ),
      h('p', 'We don\'t need a getter function, since m.x exposes any monad m\'s value. ret is its setter.'  ),
      ...
      h('button', {on: { mouseenter: update6e, mouseleave: update6l, click: updateSteps }, style: style6},
            [ cow.steps ],  ),
      h('br', ),    
      h('br', ),    
      h('button', {on: { mouseenter: update7e, mouseleave: update7l, click: updateNext }, style: style7},
            [ cow.next ],   ),
      h('p',' ',   ),
