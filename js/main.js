...
function view(m1, m2, m3, m4, m5, m6, m7, m8, mI1, mI2) { 
    ...
    [ h2('h1', {style: {textAlign: 'center', color:  '#BBFFFF'}}, 'Fun With Monads - UNDER RECONSTRUCTION'),
    ...
    h('p', ' Here is how the Monad class is defined:'),
      ...
      cow.functions1,
      h('p', 'Proposition: For all monads "m",'  ),  


      h('h3',  'How The Simple Monads Work' ),
      h('p', 'Let\'s see if the following statements are true or false when given the "Left" (addL, cubeL) or the "Right" (addR, cubeR) functions.' ),



      h('pre',  `m.ret(a).bnd(f) = f(a)`  ),
      h('p', 'Example:' ),
      h('button', {on: { mouseenter: update4e, mouseleave: update4l, click: updateDemo1 }, style: style4},
        `mM1.ret(4).bnd(cubeL) === cubeL(4,mM1)`  ),
      h('br'),  
      h('button', {on: { mouseenter: update4e, mouseleave: update4l, click: updateDemo1 }, style: style4},
        `mM1.ret(4).bnd(cubeR) === cubeR(4,mM1)`  ),
      h('br'),  
      ...
      h('pre',  `m.bnd(val => g(val).bnd(f) = m.bnd(f).bnd(g)`  ),
      h('p', 'Example:' ),
      h('button', {on: { mouseenter: update4e, mouseleave: update4l, click: updateDemo1 }, style: style4},
      ...
      h('pre',  `m.bnd(val => f(g(val)) = m.bnd(f).bnd(g)`  ),
      h('p', 'Example:' ),
      h('button', {on: { mouseenter: update4e, mouseleave: update4l, click: updateDemo1 }, style: style4},
        `mM1.bnd(val => add(val, mM1, 1).bnd(cube)) === mM1.bnd(add,1).bnd(cube)`  ),
      h('br'),  
      ...
      h('button', {on: { mouseenter: update4e, mouseleave: update4l, click: updateDemo1 }, style: style4},
        ` mM2.ret(mM1.bnd(add,1).bnd(square) === mM1.bnd(val => square(add(val, mM1, 1).x, mM1)))`  ),
...
function updateDemo1() {
  mM2.ret(mM1.bnd(add,1).bnd(square) === mM1.bnd(val => cube(square(val, mM1, 1).x, mM1)))
  update0();
}
