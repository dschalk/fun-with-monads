...
This repo is running online at [http://schalk.net:4002](http://schalk.net:4002).
...
```javascript
 'use strict'
  ...
  let block = (x,mon,mon2) => {
    mon2.flag = true;
    console.log('$$$$$$$$$$$$$$$$$$$$$$$$$$$',mon2.id,mon2.x,mon2.flag);
    return mon;
  }

  let release = (x,mon,mon2) => {
    mon2.flag = false;
    console.log('***************************',mon2.id,mon2.x,mon2.flag);
    return mon;
  }
  ...
  let test6 = () => {
    mMS1.ret(3).fmap(ad,mMS2,mMS1.x)
    .fmap(du).fmap(ad,mM1,mMS1.x)
    .fmap(cu).fmap(id,mMS3).bnd(add,mMS2.x + 1000);
  }
